...
int current_index = 0; 
char* log_filename;
char* patch_filename = "no";
sem_t empty, full;
int *buff;
//int patch_offset= 0;
pthread_mutex_t mutex1 = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutex_log = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutex_patch = PTHREAD_MUTEX_INITIALIZER;
...
string alias_check(string filename){
	int patch_file_num = open(patch_filename, O_RDONLY);
	char c = '\0';
	string hash_key_value = "";
	int i = 0;
	const char *char_new_name = filename.c_str();
	int hash_key = ((int)*char_new_name % 8000);
	while(c != '\n'){
		pread(patch_file_num, &c, 1, (hash_key*128)+i);
		if(c == '\0' && hash_key_value == ""){
			return filename;
		}
		if(i > 128){
			break;
	...
	string value = hash_key_value.substr(hash_key_value.find(':') + 1);
	if(value.substr(value.length() - 1) == "\n"){
		value = value.substr(0,value.length()-1);
	}
	return alias_check(value);
}
...
bool validity_check(string filename){
	...
	string temp = alias_check(filename);

	if(temp.length() != 27) return false;
...
void patch_create_new(int socket, string new_name, string existing_name){
	string entry = new_name + ":" + existing_name + "\n";
	int patch_file_num = open(patch_filename, O_WRONLY | O_CREAT, 0777);
	pthread_mutex_lock(&mutex_patch);
	const char *char_new_name = new_name.c_str();

	int patch_offset = ((int)*char_new_name % 8000);

	pthread_mutex_unlock(&mutex_patch);
	pwrite(patch_file_num, entry.c_str(), entry.length(), patch_offset*128);

	close(patch_file_num);
...
void patch_parse(string body, int socket){
	string existing_name;
	string new_name;
	string word;
	int count = 0;
	stringstream iss(body);
	while(iss >> word || count < 3){
		if(count == 0 && word != "ALIAS"){
			continue;
		}
		if(count == 1){
			existing_name = word;
	...
	struct stat buffer;
	if(stat (existing_name.c_str(), &buffer) == 0){
		patch_create_new(socket, new_name, existing_name);
	}
	else{
		error_print(404, socket);
	}
...
int get_put_checker(string line){
	...
	int temp_typePa = (line.find("PATCH"));
	...
	if(temp_typePa != -1 && (line.substr(temp_typePa, 5) == "PATCH")){
		return 3;
	}
	return 0;
}
	...
	while(1){
		...
		try{
			while((numbytes = recv(socket, &c, 1, 0)) != 0){ //Goes through first line of header passed in to server
				...
				if(method_type == 3 && end_header == 1){
					//if(patch_phase == 0){
					body += c;
					//}
					if(body.length() > 2 && body.substr(body.length() - 2) == "\r\n"){
						patch_parse(body, socket);
						request_type = "PATCH";
						if(l){
							logger_creation(request_type, body, body.length(), patch_filename, -1);
						}
						body = "";
						method_type = -1;
						end_header = 0;
...
int main(int argc, char * argv[]){
	...
	if(patch_filename == "no"){
		warn("No alias file designated");
		return 0;
	}
	int patch_num = open(patch_filename, O_RDONLY);
	if(patch_num == -1){
		int patch_file_num = open(patch_filename, O_WRONLY | O_CREAT, 0777);
		char *space = (char*) malloc(sizeof(char)*128);
		for(int i = 0; i < 8000; i++){
			write(patch_file_num, space, 127);
			write(patch_file_num, "\n", 1);
		}
		close(patch_file_num);
	}
